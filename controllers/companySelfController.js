...
`,
  ...
  .catch((err) =>
    console.warn("subscription_requests table init failed:", err.message),
  );
...
`,
  ...
  .catch((err) =>
    console.warn("subscription_requests index init failed:", err.message),
  );
...
async function getScheduleTimeStorageMode() {
  ...
  scheduleTimeStorageMode = departureType.includes("TIMESTAMP")
    ? "timestamp"
    : "time";
  return scheduleTimeStorageMode;
}

async function normalizeScheduleTimesForStorage(
  scheduleDate,
  departureTime,
  arrivalTime,
) {
...
async function getCompanyPlanContext(companyId) {
  ...
  const plan =
    normalizePlan(company?.plan || company?.subscription_plan) || DEFAULT_PLAN;
  return {
    company,
    plan,
...
const getCompany = async (req, res) => {
  try {
    ...
    const latestSubscriptionRequest =
      await getLatestSubscriptionRequest(companyId);
    ...
    const mapped = {
      ...
      plan:
        normalizePlan(company.plan || company.subscription_plan) ||
        DEFAULT_PLAN,
      subscriptionPlan:
        normalizePlan(company.plan || company.subscription_plan) ||
        DEFAULT_PLAN,
      ...
      planPermissions: getPlanPermissions(
        company.plan || company.subscription_plan || DEFAULT_PLAN,
      ),
  ...
  try {
    ...
    if (!requestedPlan) {
      return res
        .status(400)
        .json({
          error: "requested_plan must be Starter, Growth, or Enterprise",
        });
    }
    ...
    if (!isPlanUpgrade(currentPlan, requestedPlan)) {
      return res
        .status(400)
        .json({ error: "Only plan upgrades can be requested from this page" });
    }
    ...
    if (pendingRows[0]) {
      return res
        .status(409)
        .json({ error: "A pending request for this plan already exists" });
    }
    ...
    try {
      ...
      console.error(
        "createSubscriptionRequest notification error:",
        notificationError,
      );
    }
...
const getBuses = async (req, res) => {
  try {
    const companyId =
      req.companyId || (await User.findByPk(req.userId)).company_id;
...
const createBus = async (req, res) => {
  try {
    ...
    console.log(
      "Driver ID received:",
      req.body.driver_id || req.body.driverId || null,
    );
    ...
    if (permissions.limits.maxBuses !== null) {
      const existingBusCount = await Bus.count({
    ...
    if (payload.driver_id) {
      ...
      const driverUser = await User.findOne({
        where: { id: payload.driver_id, role: "driver" },
      });
    ...
    const bus = await busService.createBus(companyId, payload, {
      assignedBy: userId,
    });
...
const assignBusDriver = async (req, res) => {
  try {
    ...
    const bus = await busService.updateBus(companyId, busId, payload, {
      assignedBy: userId,
    });
  ...
  try {
    ...
    if (busIds.length > 0) {
      ...
      const assignments = await DriverAssignment.findAll({
        where: { bus_id: busIds, unassigned_at: null },
        include: [{ model: require("../models").Driver, required: false }],
      });
...
const getTickets = async (req, res) => {
  try {
    ...
    const ticketsTable = await sequelize
      .getQueryInterface()
      .describeTable("tickets");
    ...
    const passengerNameExpr = hasPassengerNameColumn
      ? "t.passenger_name"
      : "NULL::text";
...
const updateTicket = async (req, res) => {
  ...
  try {
    ...
    if (ticket.company_id !== companyId) {
      ...
      return res
        .status(403)
        .json({ error: "Unauthorized to update this ticket" });
    }
    ...
    if (
      status === "CANCELLED" &&
      (previousStatus === "CONFIRMED" || previousStatus === "CHECKED_IN")
    ) {
      ...
      console.log(
        `[updateTicket] Cancellation check: Departure in ${timeDiffMinutes.toFixed(2)} minutes`,
      );
      ...
      if (timeDiffMinutes < 10) {
        ...
        return res.status(400).json({
          success: false,
          error:
            "Ticket cannot be cancelled less than 10 minutes before departure",
          message:
            "Ticket cannot be cancelled less than 10 minutes before departure",
      ...
      console.log(
        `[updateTicket] ✅ Cancellation allowed: ${timeDiffMinutes.toFixed(2)} minutes before departure`,
      );
    }
      ...
      if (
        ...
        if (schedule) {
          ...
          schedule.available_seats =
            parseInt(schedule.available_seats || 0) + 1;
          schedule.booked_seats = Math.max(
            0,
            parseInt(schedule.booked_seats || 0) - 1,
          );
          await schedule.save({ transaction });

          console.log(
            `[updateTicket] ✅ Seat ${ticket.seat_number} on schedule ${ticket.schedule_id} is now AVAILABLE`,
          );
          console.log(
            `[updateTicket] Schedule ${ticket.schedule_id}: ${schedule.available_seats} available, ${schedule.booked_seats} booked`,
          );
        }
      }
    }
...
const getDrivers = async (req, res) => {
  try {
    ...
    const legacyDrivers = await Driver.findAll({
      where: { company_id: companyId },
    });
    ...
    buses.forEach((b) => {
      ...
      busesByDriver[b.driver_id].push({
        id: b.id,
    ...
    const findLegacyForUser = (user) => {
      ...
      const match = legacyDrivers.find((ld) => {
        if (
          phone &&
          ld.phone &&
          String(ld.phone).trim() === String(phone).trim()
        )
          return true;
        if (
          email &&
          ld.email &&
          String(ld.email).trim().toLowerCase() ===
            String(email).trim().toLowerCase()
        )
          return true;
        if (name && ld.name && String(ld.name).trim().toLowerCase() === name)
          return true;
        return false;
      });
    ...
    const unmatchedLegacy = (legacyDrivers || []).filter(
      (ld) => !legacyMatchedIds.has(ld.id),
    );
  ...
  try {
    ...
    if (!isLegacy) {
      ...
      if (
        ...
        driverUser.company_id === companyId
      ) {
        return res.json({
          driver: {
            id: driverUser.id,
...
const createDriver = async (req, res) => {
  try {
    ...
    if (!name || !license)
      return res
        .status(400)
        .json({ error: "Name and license number are required" });
    ...
    const legacy = await Driver.create({
      company_id: companyId,
      name,
    ...
    console.log(
      "createDriver: created user and legacy driver",
      newUser.id,
      legacy.id,
    );
    ...
    try {
      ...
      await sendEmail({
        to: newUser.email,
        subject: "Your SafariTix account",
        text: emailBody,
      });
      if (phone) {
        await sendSMS({
          to: phone,
          text: `SafariTix temporary password: ${generatedPassword}. Change it on first login.`,
        });
      }
    } catch (e) {
      console.warn(
        "Failed to send driver temporary credentials:",
    ...
    if (error && error.name === "SequelizeUniqueConstraintError") {
      const field =
        error?.errors?.[0]?.path || Object.keys(error?.fields || {})[0];
      ...
      if (field === "phone" || field === "company_id_phone") {
        return res.status(409).json({
          error:
            "A driver with this phone number already exists for this company",
        });
      }

      return res.status(409).json({
        error: "Some driver information is already in use",
      });
    }
    res.status(400).json({ error: error.message });
  }
...
const updateDriver = async (req, res) => {
  try {
    ...
    const existingLicense = await Driver.findOne({
      where: { license_number: license, id: { [Op.ne]: rawId } },
    });
    if (existingLicense)
    ...
    const existingPhone = phone
      ? await Driver.findOne({
          where: { company_id: companyId, phone, id: { [Op.ne]: rawId } },
        })
      : null;
    if (existingPhone)
      return res
        .status(400)
        .json({ error: "Phone number already in use for this company" });
...
const deleteDriver = async (req, res) => {
  try {
    ...
    if (!isLegacy) {
      ...
      if (
        ...
        const activeAssignment = await DriverAssignment.findOne({
          where: { driver_id: rawId, unassigned_at: null },
        });
        if (assignedBus || activeAssignment)
          return res
            .status(400)
            .json({
              error:
                "Cannot delete driver assigned to a bus or active assignment exists",
            });
    ...
    const assignedBusLegacy = await Bus.findOne({
      where: { driver_id: rawId },
    });
    const activeAssignmentLegacy = await DriverAssignment.findOne({
      where: { driver_id: rawId, unassigned_at: null },
    });
    if (assignedBusLegacy || activeAssignmentLegacy)
      return res
        .status(400)
        .json({
          error:
...
const createSchedule = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Bus, route, times, and date are required" });
    }

    const routeCount = await Route.count({ where: { company_id: companyId } });
    if (
      !hasPlanFeature(plan, "unlimitedRoutes") &&
      permissions.limits.maxRoutes !== null
    ) {
    ...
    if (!hasPlanFeature(plan, "advancedSchedules")) {
      ...
      if (
        permissions.limits.maxActiveSchedules !== null &&
        activeScheduleCount >= permissions.limits.maxActiveSchedules
      ) {
        return res.status(403).json({
    ...
    if (driverId) {
      ...
      if (driverUser) {
        if (
          ...
          return res
            .status(400)
            .json({ error: "Invalid driver for this company" });
        }
      } else {
    ...
    const ruaResult = await sequelize.query(
      ...
      {
        replacements: { from: routeFrom, to: routeTo },
        type: QueryTypes.SELECT,
      },
    );
    if (!ruaResult.length) {
      return res
        .status(400)
        .json({
          error: `No active RURA route found for ${routeFrom} → ${routeTo}. Price cannot be set manually.`,
        });
    }
  ...
  try {
    ...
    if (bus_plate_number) {
      const bus = await Bus.findOne({
        where: { plate_number: bus_plate_number, company_id: companyId },
      });
      if (bus) {
...
const deleteSchedule = async (req, res) => {
  try {
    ...
    if (bookingCount > 0) {
      return res
        .status(400)
        .json({ error: "Cannot delete schedule with existing bookings" });
    }
...
const getScheduleJournals = async (req, res) => {
  try {
    ...
    const scheduleId = req.params.id;
    const ScheduleJournal = require("../models/ScheduleJournal");
    const journals = await ScheduleJournal.findAll({
      where: { schedule_id: scheduleId, company_id: companyId },
...
const patchBusStatus = async (req, res) => {
  try {
    ...
    const bus = await busService.setStatus(companyId, busId, status, {
      updatedBy: userId,
    });
    if (!bus) return res.status(404).json({ error: "Bus not found" });
...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const planContext = companyId
      ? await getCompanyPlanContext(companyId)
      : null;
    ...
    const last30DaysTickets = allTickets.filter(
      (t) => new Date(t.booked_at) >= thirtyDaysAgo,
    );
    const last30DaysRevenue = last30DaysTickets.reduce(
    ...
    const previous30DaysRevenue = previous30DaysTickets.reduce(
      (sum, t) => sum + parseFloat(t.price || 0),
      0,
    );
    ...
    const salesCountGrowth =
      previous30DaysTickets.length > 0
        ? (
            ((last30DaysTickets.length - previous30DaysTickets.length) /
              previous30DaysTickets.length) *
            100
          ).toFixed(1)
        : 0;
    ...
    sortedRoutes.forEach(([route, revenue], index) => {
      const percentage =
        totalRevenue > 0 ? ((revenue / totalRevenue) * 100).toFixed(0) : 0;
    ...
    if (otherRevenue > 0) {
      const percentage =
        totalRevenue > 0 ? ((otherRevenue / totalRevenue) * 100).toFixed(0) : 0;
    ...
    const responseData = {
      ...
      planPermissions:
        planContext?.permissions || getPlanPermissions(DEFAULT_PLAN),
    };
...
const getActiveTrips = async (req, res) => {
  try {
    ...
    console.log(
      `Returning ${activeTrips.length} active trips for company ${companyId}`,
    );
...
const getRevenue = async (req, res) => {
  try {
    ...
    const todayTicketsList = activeTickets.filter(
      (row) => toDateString(row.effective_date) === todayStr,
    );
    ...
    const todayRevenue = todayTicketsList.reduce(
      (sum, row) => sum + Number(row.price || 0),
      0,
    );
    const weekRevenue = weekTicketsList.reduce(
      (sum, row) => sum + Number(row.price || 0),
      0,
    );
    const monthRevenue = monthTicketsList.reduce(
      (sum, row) => sum + Number(row.price || 0),
      0,
    );
    ...
    activeTickets.forEach((row) => {
      ...
      if (!dailyMap.has(date))
        dailyMap.set(date, { date, revenue: 0, tickets: 0 });
      ...
      const hour =
        departureTime !== "—"
          ...
          : String(new Date(row.booked_at).getHours()).padStart(2, "0");
    ...
    const totalCapacity = Array.from(occupancyMap.values()).reduce(
      (sum, entry) => sum + Number(entry.capacity || 0),
      0,
    );
    const totalSoldForOccupancy = Array.from(occupancyMap.values()).reduce(
      (sum, entry) => sum + Number(entry.sold || 0),
      0,
    );
    const occupancyRate =
      totalCapacity > 0 ? (totalSoldForOccupancy / totalCapacity) * 100 : 0;

    const averageTicketPrice =
      totalTickets > 0 ? totalRevenue / totalTickets : 0;

    const previousPeriodMetrics = (() => {
      const dateBuckets = activeTickets
        .map((row) => toDateString(row.effective_date))
        .filter(Boolean);
      ...
      const currentRows = activeTickets.filter((row) =>
        inRange(
          toDateString(row.effective_date),
          currentStartStr,
          currentEndStr,
        ),
      );
      const previousRows = activeTickets.filter((row) =>
        inRange(toDateString(row.effective_date), prevStartStr, prevEndStr),
      );
    ...
    const breakdownByRoute = Array.from(routeBreakdownMap.values()).sort(
