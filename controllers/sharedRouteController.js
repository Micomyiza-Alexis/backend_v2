...
const findStopSequence = (stops, stopName) => {
  ...
  const match = stops.find(
    (item) => normalizeStopName(item.stop_name) === normalized,
  );
...
const resolveRoutePoint = (name, fallback) => {
  ...
  const partial = Object.entries(ROUTE_COORDINATE_HINTS).find(([key]) =>
...
const buildGuestDemoLocation = ({
  ...
  const departureIso = scheduleDate
    ...
    : null;
  const departureTimestamp = departureIso
    ? new Date(departureIso).getTime()
    : Date.now() - 15 * 60 * 1000;
  const elapsedMs = Math.max(
    0,
    Date.now() -
      (Number.isFinite(departureTimestamp) ? departureTimestamp : Date.now()),
  );
  ...
  return {
    ...
    currentLocationLabel:
      fromName && toName ? `Between ${fromName} and ${toName}` : "On route",
  };
};

const getLatestGuestLocationForBooking = async (
  client,
  scheduleId,
  busId,
  routeFrom,
  ...
  if (latest) {
    ...
    const distanceRemainingKm = destination
      ? calculateDistanceKm(currentPoint, destination)
      : null;
    const etaMinutes =
      speed && speed > 0 && distanceRemainingKm !== null
...
const ensureMobilePassenger = async ({ email, passengerName, phoneNumber }) => {
  ...
  const normalizedName =
    String(passengerName || "").trim() || "Mobile Passenger";
  ...
  if (user) {
    ...
    if (normalizedName && (!user.full_name || user.full_name === user.email))
...
const getScheduleOccupancy = async (
  client,
  scheduleId,
  routeStops,
  fromStop,
  ...
  const routeMaxSeq = routeStops.length
    ? toInt(routeStops[routeStops.length - 1].sequence, 0)
    : 0;
  const ticketsColumns = await getTableColumns(client, "tickets");
  const hasSequenceColumns =
    ticketsColumns.has("from_sequence") && ticketsColumns.has("to_sequence");
  ...
  if (hasSequenceColumns) {
    ...
    return {
      occupiedSeats: new Set(
        occupied.rows.map((row) => String(row.seat_number)),
      ),
      fromSeq,
      toSeq,
    };
  ...
  for (const ticket of ticketsResult.rows) {
    ...
    if (ticketFrom === null)
      ticketFrom = findStopSequence(routeStops, ticket.from_stop);
    if (ticketTo === null)
      ticketTo = findStopSequence(routeStops, ticket.to_stop);
  ...
  try {
    ...
    if (effectiveDate) {
      if (!isValidDate(effectiveDate)) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid effective_date filter" });
      }
      params.push(effectiveDate);
      where.push(`r.effective_date::date <= $${params.length}::date`);
    }
    if (companyId) {
      const ruraColumns = await getTableColumns(
      ...
      if (ruraColumns.has("company_id")) {
        ...
        where.push(
          `(r.company_id::text = $${params.length} OR r.company_id IS NULL)`,
        );
      }
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch shared routes" });
  } finally {
...
const getRouteStops = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch route stops" });
  } finally {
    if (client) client.release();
  }
};
...
const upsertRouteStops = async (req, res) => {
  ...
  try {
    ...
    if (!stops.length) {
      return res
        .status(400)
        .json({ success: false, message: "At least one stop is required" });
    }
    ...
    if (!tables.rows.length) {
      ...
      return res.status(400).json({
        success: false,
        message:
          "route_stops table does not exist. Run migration 20260304_shared_route_booking.sql first.",
      });
    }

    await client.query(
      "DELETE FROM route_stops WHERE route_id::text = $1::text",
      [routeId],
    );

    for (const stop of stops) {
      const stopName = (stop.stop_name || stop.stopName || "")
        .toString()
        .trim();
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to save route stops" });
  } finally {
    if (client) client.release();
  }
};
...
const createSharedSchedule = async (req, res) => {
  ...
  try {
    ...
    const busResult = await client.query(
      "SELECT id, capacity, company_id, status FROM buses WHERE id::text = $1::text",
      [bus_id],
    );
    ...
    if (
      companyId &&
      bus.company_id &&
      String(bus.company_id) !== String(companyId)
    ) {
      return res
        .status(403)
        .json({ success: false, message: "Invalid bus for this company" });
    }

    if (String(bus.status || "").toUpperCase() !== "ACTIVE") {
      return res
        .status(400)
        .json({ success: false, message: "Cannot schedule an inactive bus" });
    }
    const routeStops = await getStopsByRoute(client, route_id);

    if (routeStops.length < 2) {
      return res.status(400).json({
        success: false,
        message:
          "Please manage at least 2 stops for this route before creating a schedule",
      });
    }
    ...
    if (tableName === "bus_schedules") {
      const inserted = await client.query(
        `
          ...
          RETURNING schedule_id, bus_id, route_id, date::text AS date, time::text AS time, capacity, company_id, status
        `,
        [
          bus_id,
          route_id,
      ...
      return res
        .status(201)
        .json({ success: true, schedule: inserted.rows[0] });
    }
    ...
    const inserted = await client.query(
      `
        ...
        RETURNING id AS schedule_id, bus_id, route_id, schedule_date::text AS date, departure_time::time::text AS time, total_seats AS capacity, status
      `,
      [
        bus_id,
        route_id,
...
const listSharedSchedules = async (req, res) => {
  ...
  try {
    ...
    const status = (req.query.status || "").toString().trim().toLowerCase();
    const companyId =
      req.companyId || (req.query.company_id || "").toString().trim();
    ...
    if (date) {
      if (!isValidDate(date))
        return res
          .status(400)
          .json({ success: false, message: "Invalid date filter" });
      params.push(date);
      ...
      tableName === "bus_schedules"
        ...
        : `
            SELECT
              ...
              bs.departure_time::time::text AS time,
...
const searchSharedSchedules = async (req, res) => {
  ...
  try {
    ...
    if (fromSeq < 0 || toSeq < 0 || fromSeq >= toSeq) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid from_stop/to_stop segment" });
    }
    ...
    for (const schedule of schedulesResult.rows) {
      ...
      for (let seat = 1; seat <= capacity; seat += 1) {
        if (!occupancy.occupiedSeats.has(String(seat)))
          availableSeatNumbers.push(seat);
      }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to search schedules" });
  } finally {
    if (client) client.release();
  }
};
...
const searchTrips = async (req, res) => {
  let client;
  try {
    const from = (req.query.from || req.query.from_stop || "")
      .toString()
      .trim();
    ...
    for (const routeMatch of matchingRoutes.rows) {
      ...
      const dateCol =
        tableName === "bus_schedules" ? "bs.date" : "bs.schedule_date";
      const timeCol =
        tableName === "bus_schedules" ? "bs.time" : "bs.departure_time";
      if (date) {
      ...
      const orderCol =
        tableName === "bus_schedules"
          ? "bs.date, bs.time"
          : "bs.schedule_date, bs.departure_time";
      ...
      let segmentPrice = await getSegmentFare(
        client,
      ...
      if (segmentPrice === null && routeStops.length >= 2) {
        ...
        if (routeBasePrice > 0) {
          ...
          const segmentsTraveled =
            routeMatch.to_sequence - routeMatch.from_sequence;
          segmentPrice = Math.round(
            (routeBasePrice * segmentsTraveled) / totalSegments,
          );
        }
      }
...
const getAvailableSeats = async (req, res) => {
  let client;
  try {
    const scheduleId = (req.query.schedule_id || req.query.scheduleId || "")
      .toString()
      .trim();
    const fromStop = (req.query.from || req.query.from_stop || "")
      .toString()
      .trim();
    ...
    const scheduleDeparted = await hasScheduleDeparturePassed(
      client,
      schedule.date,
      schedule.time,
    );
    ...
    if (routeStops.length < 2) {
      return res
        .status(400)
        .json({ success: false, message: "Route stops are not configured" });
    }
...
const bookSharedTicket = async (req, res) => {
  ...
  try {
    ...
    if (payment_id) {
      ...
      if (!paymentCheck.rows.length) {
        return res.status(404).json({
          success: false,
          message:
            "Payment not found. Cannot book ticket without valid payment.",
        });
      }

      const payment = paymentCheck.rows[0];
      const isPaid =
        payment.booking_status === "paid" ||
        payment.booking_status === "PAID" ||
      ...
      return res.status(402).json({
        success: false,
        message:
          "Cannot create ticket: payment_id is required. Strict rule: tickets require confirmed payment.",
      });
    }
    ...
    if ((schedule.status || "").toLowerCase() === "cancelled") {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Schedule is cancelled" });
    }
    if ((schedule.bus_status || "").toLowerCase() !== "active") {
      await client.query("ROLLBACK");
      return res
        .status(400)
        .json({ success: false, message: "Assigned bus is not active" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid boarding segment" });
    }
    ...
    const seatMetaByNumber = new Map(
      seatInventory.rows.map((row) => [
        String(row.seat_number),
        Boolean(row.is_driver),
      ]),
    );
    ...
    if (selectedSeat && seatMetaByNumber.get(selectedSeat) === true) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Driver seat cannot be booked" });
    }
    ...
    if (!selectedSeat) {
      for (let seat = 1; seat <= capacity; seat += 1) {
        ...
        if (
          !occupancy.occupiedSeats.has(candidate) &&
          seatMetaByNumber.get(candidate) !== true
        ) {
    ...
    const segPrice = await getSegmentFare(
      client,
      from_stop,
      to_stop,
      schedule.date,
    ...
    add(
      "booking_ref",
      `SHR-${Date.now()}-${Math.floor(Math.random() * 10000)}`,
    );
    ...
    (async () => {
      try {
        ...
        if (userResult.rows.length) {
          ...
          const depDate = schedule.date
            ? String(schedule.date).slice(0, 10)
            : "";
          const depTime = schedule.time
            ? String(schedule.time).slice(0, 5)
            : "";
    ...
    if (passengerId) {
      (async () => {
        try {
          ...
          const commuterName =
            userRes.rows[0]?.full_name || passenger_name || "Passenger";
          ...
          if (driverId) {
            await NotificationService.createNotification(
              ...
              "ticket_booked",
              {
                relatedId: ticket.ticket_id || ticket.id,
                relatedType: "ticket",
              },
            );
          }
    ...
    if (error && error.code === "23P01") {
      return res.status(409).json({
        success: false,
        message:
          "Selected seat is no longer available for this segment. Please choose another seat.",
      });
    }
...
const confirmMobilePayment = async (req, res) => {
  ...
  try {
    ...
    const selectedSeats = Array.from(
      new Set(rawSeats.map((seat) => String(seat).trim()).filter(Boolean)),
    );
    ...
    addPaymentCol(
      ...
      JSON.stringify({
        ...
        to_stop,
        passenger_name:
          passenger_name || passenger.full_name || "Mobile Passenger",
    ...
    for (const rawSeat of selectedSeats) {
      ...
      add(
        "trip_date",
        schedule.date ? String(schedule.date).slice(0, 10) : null,
      );
      add("from_stop", from_stop);
      add("to_stop", to_stop);
      add("from_sequence", occupancy.fromSeq);
      ...
      add(
        "booking_ref",
        `MOB-${Date.now()}-${Math.floor(Math.random() * 100000)}`,
      );
      ...
      if (
        !insertCols.includes("passenger_id") ||
        !insertCols.includes("schedule_id") ||
        !insertCols.includes("seat_number") ||
      ...
      const ticketQrCodeUrl = await QRCode.toDataURL(
        JSON.stringify(ticketQrData),
        {
    ...
    sendETicketEmail({
      ...
      console.error(
        "[confirmMobilePayment] e-ticket email error:",
        err.message,
      );
    });
...
const getGuestTickets = async (req, res) => {
  ...
  try {
    ...
    const bookingRef = String(
      req.query.bookingRef ||
        req.query.booking_ref ||
        req.body?.bookingRef ||
    ...
    if (!bookingResult.rows.length) {
      return res.status(404).json({
        success: false,
        message:
          "No booking found for the provided email and booking ID. Please check the email and reference number match your confirmation.",
      });
    }
    ...
    const tickets = ticketResult.rows.map((ticket) => {
      const qrData = {
        ...
        date: ticket.schedule_date
          ? String(ticket.schedule_date).slice(0, 10)
          : null,
      ...
      return {
        ...
        departureDate: ticket.schedule_date
          ? String(ticket.schedule_date).slice(0, 10)
          : null,
        departureTime: ticket.departure_time
          ? String(ticket.departure_time).slice(0, 5)
          : null,
...
const getGuestBookingLocation = async (req, res) => {
  ...
  try {
    ...
    if (!ticket) {
      return res
        .status(404)
        .json({ success: false, message: "Ticket not found for booking" });
    }
...
const updateSharedScheduleStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to update schedule status" });
  } finally {
    if (client) client.release();
  }
};
